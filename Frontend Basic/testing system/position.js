var positions = []; // mảng chứa positions
var v_idUpdate = -1;
var vTheme = "";
var baseUrl = "http://localhost:8080/api/v1/positions";
var baseAvt =
...
function loadData() {
    // call api đến mockapi.io đe lấy ds position
    // jqAjax
    $.ajax({
        type: "GET",
        url: baseUrl,
...
function onCreate(idDelete) {
    ...
    //https://images2.thanhnien.vn/528068263637045248/2024/1/25/e093e9cfc9027d6a142358d24d2ee350-65a11ac2af785880-17061562929701875684912.jpg
    // call api dể thêm mới position
    $.ajax({
        type: "POST",
        url: baseUrl,
        data: JSON.stringify(position), // chuyển account từ obejct của JS thành JSON
        contentType: "application/json",
        success: function (response) {
            alert("Thêm dữ liệu thành công");
            // hiển thị lại ds position
            loadData();
...
function onUpdate(idDelete) {
    ...
    $.ajax({
        ...
        data: JSON.stringify(positionUpdate),
        contentType: "application/json",
        success: function (response) {
            alert("Update dữ liệu thành công");
            // hiển thi ls position
            loadData();
            //jqValSet
            v_idUpdate = -1;
            $("#inputAvatar").val("");
